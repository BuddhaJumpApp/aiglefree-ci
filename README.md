...
三件套：`fjolskylduoryggisverndar/aiglefree`（源码，私有）→ **`BuddhaJumpApp/aiglefree-ci`（本仓库，公开）** → `fjolskylduoryggisverndar/aiglefree-web`（官网快照，域名 `aiglefree.xyz`）。
...
- 发送方必须向 `api.github.com/repos/BuddhaJumpApp/aiglefree-ci/dispatches` POST `event_type: pubspec-updated`。
...
gh workflow run build.yml --repo BuddhaJumpApp/aiglefree-ci --ref master
...
# 看进度
gh run list --repo BuddhaJumpApp/aiglefree-ci --workflow build.yml --limit 5
gh run watch --repo BuddhaJumpApp/aiglefree-ci <run-id>
```
...
官网 → `dl.aiglefree.xyz/android|windows` → Cloudflare Worker `fjolsky-downloads` → 302 → `edgeone.gh-proxy.org/https://github.com/BuddhaJumpApp/aiglefree-ci/releases/download/<tag>/<asset>`。
